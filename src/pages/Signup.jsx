...
function Signup() {
  ...
  const create = async (data) => {
    ...
    try {
      ...
      if (msg.toLowerCase().includes('already exists') || msg.toLowerCase().includes('already been used')) {
        setError('An account with this email already exists. Try signing in instead.')
      } else if (msg.toLowerCase().includes('password')) {
        setError('Password is too weak or too common. Use at least 8 characters with a mix of letters and numbers.')
      } else {
        setError(msg)
      }
    }
  ...
  const fieldError = (msg) => (
    <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.78rem', color: '#dc2626', marginTop: '6px', display: 'flex', alignItems: 'center', gap: '4px' }}>
      <span>⚠</span> {msg}
    </p>
  )
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ width: '100%', maxWidth: '400px' }} className="animate-fade-up">
          ...
          <div style={{ marginBottom: '36px' }}>
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: isMobile ? '2rem' : '2.2rem', fontWeight: 700, color: 'var(--ink)', letterSpacing: '-0.03em', marginBottom: '10px' }}>
              Join Inkwell.
            </h1>
          ...
          {error && (
            <div style={{ background: '#fef2f2', border: '1px solid #fecaca', color: '#dc2626', padding: '12px 16px', borderRadius: '8px', fontSize: '0.875rem', marginBottom: '24px', fontFamily: 'var(--font-body)', display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
          ...
          <form onSubmit={handleSubmit(create)}>
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <button
                    ...
                    style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ink-muted)', padding: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'color 0.2s ease' }}
                ...
                {passwordValue.length > 0 && (
                  <div style={{ marginTop: '10px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.68rem', color: strength.color }}>
                        {strength.label}
                      </span>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                      {[
                        { rule: passwordValue.length >= 8, text: 'At least 8 characters' },
                        { rule: /[A-Z]/.test(passwordValue), text: 'One uppercase letter' },
                        { rule: /[0-9]/.test(passwordValue), text: 'One number' },
                      ].map(({ rule, text }) => (
                        <div key={text} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          <span style={{ fontSize: '0.7rem', color: rule ? '#22c55e' : 'var(--ink-faint)', transition: 'color 0.2s ease' }}>
                            {rule ? '✓' : '○'}
                          </span>
                          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: rule ? '#22c55e' : 'var(--ink-faint)', transition: 'color 0.2s ease' }}>
                            {text}
                          </span>
                        </div>
                      ))}
            ...
            <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.78rem', color: 'var(--ink-faint)', textAlign: 'center', marginTop: '16px' }}>
      ...
      {!isMobile && (
        <div style={{
          ...
          <div style={{ position: 'absolute', inset: 0, backgroundImage: `radial-gradient(circle at 70% 30%, rgba(200,81,42,0.15) 0%, transparent 50%), radial-gradient(circle at 20% 80%, rgba(184,149,58,0.08) 0%, transparent 40%)` }} />
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            <blockquote style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.6rem, 2.8vw, 2.4rem)', fontWeight: 700, color: 'var(--paper)', lineHeight: 1.2, letterSpacing: '-0.03em', marginBottom: '20px' }}>
              "There is no greater agony than bearing an untold story inside you."
            </blockquote>
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.72rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(245,242,235,0.35)' }}>
              — Maya Angelou
            </p>
          </div>

          <div style={{ position: 'relative', zIndex: 1 }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.68rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--accent)', marginBottom: '20px' }}>Why Inkwell?</div>
            {[
              ...
              <div key={item.text} style={{ display: 'flex', gap: '14px', alignItems: 'flex-start', marginBottom: '14px' }}>
                <span style={{ color: 'var(--accent)', fontSize: '0.75rem', marginTop: '3px' }}>{item.icon}</span>
                <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.9rem', color: 'rgba(245,242,235,0.65)', lineHeight: 1.5 }}>{item.text}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ position: 'absolute', bottom: 0, right: 0, width: '180px', height: '180px', border: '1px solid rgba(255,255,255,0.04)', borderRadius: '50%', transform: 'translate(30%, 30%)' }} />
        </div>
      )}
    </div>
  )
